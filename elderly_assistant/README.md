...
- **扫码识别药品**：点击主界面「扫码查药」触摸按钮，打开摄像头扫描药盒条码，按药品编号匹配用药计划后语音播报「药品名 + 用量 + 服药时间」。支持两条通路：HuskyLens 板载条码解码（默认，仅条码识别，无需本机解码）与 USB 摄像头 + pyzbar 本地解码（自动回退）。
...
3. 系统在 `SCAN_TIMEOUT_SEC` 秒内轮询扫码：优先 HuskyLens 板载条码（算法 17）识别，不可用时自动回退 USB 摄像头 + pyzbar 本地解码。**HuskyLens 仅启用条码识别，不再调用二维码识别（算法 18）。**
