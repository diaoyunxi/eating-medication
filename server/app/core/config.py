...
def _write_full_env(env_path: Path, secret_key: str):
    ...
    env_content = (
        ...
        f"# 远程 MySQL：  mysql+pymysql://user:password@host:3306/dbname\n"
        f"# 远程 PostgreSQL：postgresql+psycopg2://user:password@host:5432/dbname\n"
        f"# 检测到数据库不存在时会自动建库（MySQL/PostgreSQL），无需手动创建。\n"
