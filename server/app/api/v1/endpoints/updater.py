...
则下载完整发布包、做 Release Attestation 校验（使用 gh CLI 验证 GitHub 官方签名，
验证失败时拒绝自动更新）并安全复制到项目根目录
（保留 .env / data / logs 等保护文件），更新成功后自动重启服务。

无需鉴权：便于 CI / 部署脚本 / 浏览器直接访问触发自更新。

底层更新逻辑复用仓库根目录的 updater.py（安全自动更新 / Release Attestation 校验 / 保护文件）。
"""
import asyncio
import logging
...
async def updater_status():
    ...
    做 Release Attestation 校验（使用 gh CLI 验证 GitHub 官方签名，
    验证失败则出于安全考虑拒绝更新）并安全复制到项目根目录
    （保留 .env / data / logs 等保护文件）。
