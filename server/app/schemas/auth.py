...
class RegisterReq(BaseModel):
    ...
    username: str | None = Field(None, max_length=50, description="昵称（可选）")
    password: str = Field(..., min_length=6, max_length=100, description="密码")
    role: str = Field(..., description="角色: elderly 或 family")
    # Cloudflare Turnstile 人机验证令牌（前端提交，后端调 siteverify 校验）
    cf_turnstile_token: str | None = Field(None, description="Cloudflare Turnstile 令牌")
    # GitHub/Gitee OAuth 补全注册时携带的短期身份令牌（由 server 回调签发）；为空表示普通注册
    oauth_token: str | None = Field(None, description="GitHub/Gitee OAuth 身份令牌")
