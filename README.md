# 老人用药管理智能助手（ICC Project）

> 版本：v2.0.0 | 仓库：[diaoyunxi/eating-medication](https://github.com/diaoyunxi/eating-medication)
...
各模块版本统一由 `VERSION` 文件管理，当前为 **v2.0.0**。
...
python main.py             # 启动服务（支持 HTTPS，需放置证书文件）
```

> **注意**：`.env`、证书文件（`certs/`）、数据库（`*.db`）等敏感文件已通过 `.gitignore` 排除，不会上传至仓库，部署时需自行配置。
...
| 模块 | 配置文件 | 关键配置项 |
|------|----------|------------|
| elderly_assistant | `config.yaml` | 服务器地址、AI 接口、摄像头、语音引擎、蜂鸣器、提醒升级间隔 |
| server | `.env` | 数据库、密钥、智谱 AI、京东比价 |
| family_monitor | `.env` + `config.json` | 服务器地址、会话密钥、SSL 证书路径 |
