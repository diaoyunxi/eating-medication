...
#
# 老人端（行空板 M10）特殊处理：
#   - 纯文件运行模式，不安装 systemd 服务、不配置 sudoers、不配置 cloudflared/DDNS/Caddy
#   - 仅安装系统依赖 + 克隆仓库 + 虚拟环境 + elderly_assistant 依赖 + 生成 .env
#   - 无交互按键等待，安装完成后提示手动启动
#
# 用法：
...
check_gh_cli() {
    if command -v gh >/dev/null 2>&1; then
        ...
        log_warn "安装 GitHub CLI：https://cli.github.com/ （apt: gh / brew: gh / dnf: gh）"
    fi
}
check_gh_cli
...
setup_python_env() {
    ...
    # 老人端以 root 运行，venv 保持 root 所有；服务端改属 deploy 用户
    if [ "${ELDERLY_MODE:-0}" = "0" ]; then
...
setup_runtime_dirs() {
    ...
    if [ "${ELDERLY_MODE:-0}" = "1" ]; then
        # 老人端纯文件运行，以 root 运行，无需 chown 给 deploy 用户
        $SUDO mkdir -p \
...
setup_systemd_services() {
    # 老人端：纯文件运行模式，不安装 systemd 服务、不配置 sudoers
    if [ "${ELDERLY_MODE:-0}" = "1" ]; then
        log_info "老人端为纯文件运行模式，跳过 systemd 服务安装与 sudoers 配置"
        log_info "安装完成后请手动启动: cd ${DEPLOY_DIR}/elderly_assistant && ${VENV}/bin/python main.py"
        return 0
    fi
...
# ============================================================
prompt_edit_env() {
    # 老人端：纯文件运行模式，无按键交互，提示手动启动
    if [ "${ELDERLY_MODE:-0}" = "1" ]; then
        log_step "[9/9] 提示编辑配置文件"
        sleep 1
        printf '\n'
        printf '============================================================\n'
        printf '  老人端部署完成（纯文件运行模式，无服务安装）\n'
        printf '============================================================\n'
        printf '\n'
        ...
        printf '     （不修改则使用程序内置默认值，可直接启动）\n'
        printf '\n'
        log_info "手动启动命令: cd ${DEPLOY_DIR}/elderly_assistant && ${VENV}/bin/python main.py"
        return 0
    fi
...
uninstall() {
    ...
    # 老人端不安装 systemd 服务，仅检测部署目录与用户
    if [ "${ELDERLY_MODE:-0}" = "0" ]; then
        for _svc in eating-medication-server.service eating-medication-family.service; do
    ...
    # 1. 停止并禁用 systemd 服务（仅服务端，老人端为纯文件运行模式无服务）
    if [ "${ELDERLY_MODE:-0}" = "0" ]; then
        ...
        log_step "[1/3] 老人端无需停止服务（纯文件运行模式）"
    fi

    # 2. 删除 systemd 服务文件
    # 2. 删除 systemd 服务文件（仅服务端）
    if [ "${ELDERLY_MODE:-0}" = "0" ]; then
        ...
        log_step "[2/3] 老人端无服务文件与 sudoers 规则，跳过"
    fi
    ...
    if [ "${ELDERLY_MODE:-0}" = "0" ]; then
        ...
        printf '    - 仅部署目录（老人端纯文件运行，无服务/用户）\n'
    fi
    printf '\n'
    printf '  保留（未删除）:\n'
    printf '    - 系统包: git, python3, curl\n'
    printf '%s\n' '------------------------------------------------------------'
}
...
main() {
    ...
    if is_elderly_device; then
        ELDERLY_MODE=1
        log_info "检测到老人端设备（行空板 M10）：纯文件运行模式"
        log_info "  - 仅安装 elderly_assistant + 系统工具"
        log_info "  - 跳过: cloudflared/DDNS/Caddy、systemd 服务、sudoers、运行用户"
    else
        ELDERLY_MODE=0
    ...
    if [ "${ELDERLY_MODE:-0}" = "1" ]; then
        printf '\n'
        printf '%s\n' '------------------------------------------------------------'
        printf '  后续步骤（老人端，纯文件运行模式）:\n'
        printf '  1) 启动应用:\n'
        printf '     cd %s/elderly_assistant && %s/bin/python main.py\n' "$DEPLOY_DIR" "$VENV"
        printf '  2) 更新代码（安全自动更新，保留 .env/data）:\n'
        printf '     cd %s && %s/bin/python updater.py\n' "$DEPLOY_DIR" "$VENV"
        printf '     或强制更新: cd %s && %s/bin/python updater.py --force\n' "$DEPLOY_DIR" "$VENV"
